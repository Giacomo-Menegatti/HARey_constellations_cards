...
def load_stars(filename, constellations=None):
    ...
    stars_df = pd.read_csv(filename, sep='|',  usecols=[1,3,4,5], header = None, na_values=['     ', '       ', '        ', '            '])
    stars_df.columns = ('hip','ra', 'dec', 'magnitude')
    ...
    #If the constellations are already been computated, add the constellation to the star
